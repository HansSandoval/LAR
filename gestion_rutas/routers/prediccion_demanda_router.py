...
from ..models.models import PrediccionDemanda, Zona
...
async def get_predicciones_demanda(
    ...
    horizonte_horas: int = Query(None, description="Filtrar por horizonte de predicción (horas)"),
    ...
    modelo_version: str = Query(None, description="Filtrar por versión del modelo LSTM"),
    ...
    **Ejemplo de uso:**
    ...
    """
    try:
        query = db.query(PrediccionDemanda)
        
        # Aplicar filtros
        if id_zona:
            query = query.filter(PrediccionDemanda.id_zona == id_zona)
        if horizonte_horas:
            query = query.filter(PrediccionDemanda.horizonte_horas == horizonte_horas)
        if fecha_desde:
            query = query.filter(PrediccionDemanda.fecha_prediccion >= fecha_desde)
        if fecha_hasta:
            query = query.filter(PrediccionDemanda.fecha_prediccion <= fecha_hasta)
        if modelo_version:
            query = query.filter(PrediccionDemanda.modelo_lstm_version == modelo_version)
        
        # Obtener total
        total = query.count()
        
        # Aplicar paginación
        predicciones = query.offset(skip).limit(limit).all()
        
        return {
            "data": [PrediccionDemandaResponse.from_orm(p) for p in predicciones],
            "total": total,
            "skip": skip,
            "limit": limit
        }
